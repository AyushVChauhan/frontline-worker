...
const QuizScreenFaceDetection = () => {
    ...
    useEffect(() => {
        ...
        const appEvent = AppState.addEventListener("change", (newState) => {
            if (newState == "active") {
                setComp(
                    <Camera
                        style={{ width: 1, height: 1 }}
                ...
                fetch(
                    `http://${ipadd.ip}:3000/worker/disqualifyBackground?token=${auth.token}&quizId=${quizAuth.quizId}&reason=3`
                );
                setComp(null);
                Alert.alert("Restricted Action");
            }
        });
        const backAction = () => {
    ...
    useEffect(() => {
        if (questionNoBarRef.current) {
            ...
            questionNoBarRef.current.scrollTo({ x: scrollOffset, animated: true });
        }
    }, [currentQuestion]);
    ...
    function faceDetection({ faces }) {
        //2 faces -> backend send photo
        if (faces.length >= 2) {
            disqualify("MULTIPLE FACES DETECTED");
        }
        //0 face -> 2 warning (useState)
        if (faces.length == 0) {
            disqualify("NO FACE DETECTED");
        }
        //different face
        if (faces.length == 1) {
            if (faceId == -1) {
                ...
                if (faceId != faces[0].faceID) {
                    faceId = faces[0].faceID;
                    disqualify("DIFFERENT FACE DETECTED");
                }
            }
        }
        //first set id
        //click photo -> send backend -> setFaceId
    }
    ...
    async function disqualify(reason) {
        if (warning > 2) {
            ...
            if (reason == "MULTIPLE FACES DETECTED") {
                reasonNum = 2;
            } else if (reason == "DIFFERENT FACE DETECTED") {
                reasonNum = 1;
            }
            if (cameraRef.current) {
                ...
                const response = FileSystem.uploadAsync(
                    `http://${ipadd.ip}:3000/worker/disqualifyPhoto?token=${auth.token}&quizId=${quizAuth.quizId}&reason=${reasonNum}`,
                    photo.uri,
                    {
                        fieldName: "photo",
                        httpMethod: "POST",
            ...
            warning++;
            Alert.alert(reason, "Instructions");
        }
    }
...
export default QuizScreenFaceDetection;
