...
import { setTraining } from "../../store/slices/CertificateSlice";
...
const CertificateScreen = () => {
    ...
    async function fetchData() {
        let response = await fetch(
            `http://${ipadd.ip}:3000/worker/trainingCertificate?token=${auth.token}`
        );
        let data = await response.json();
        // console.log(data.topics);
        dispatch(setTraining(data.certificates));
        setLoading(false);
    }
    function refresh() {
        setLoading(true);
