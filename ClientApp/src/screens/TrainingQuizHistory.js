...
const TrainingQuizHistory = ({ navigation }) => {
    ...
    async function fetchData() {
        let response = await fetch(
            `http://${ipadd.ip}:3000/worker/history?token=${auth.token}&type=1`
        );
        let data = await response.json();
        // console.log(data.topics);
        dispatch(setHistoryTraining(data.myHistory));
        setLoading(false);
    }
    function refresh() {
        setLoading(true);
...
export default TrainingQuizHistory;
