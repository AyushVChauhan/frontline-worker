...
const FinalTestHistory = ({ navigation }) => {
    ...
    async function fetchData() {
        let response = await fetch(
            `http://${ipadd.ip}:3000/worker/history?token=${auth.token}&type=2`
        );
        let data = await response.json();
        // console.log(data.topics);
        dispatch(setHistoryFinal(data.myHistory));
        setLoading(false);
    }
    function refresh() {
        setLoading(true);
    ...
    const renderQuizHistoryCard = ({ item }) => {
        async function fetchData(sessionId) {
            ...
            let data = await response.json();
            // console.log(data.topics);
            // console.log(data.quiz, data.quiz._id);
            dispatch(
                setCurrentQuiz({
        ...
        return (
            <View style={styles.card}>
                ...
                <Text style={styles.quizName}>{item.subject}</Text>
                ...
                {item.status == 0 || item.status == 2 || new Date(item.valid_to) - new Date() > 0 ? (
                    <TouchableOpacity
...
export default FinalTestHistory;
