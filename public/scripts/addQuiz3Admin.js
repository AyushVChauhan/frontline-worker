...
moduleData.forEach(element=>{
	element[1].module.forEach(module=>{
		allModules.push({module:module.moduleName, count : 0});
	})
})
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            html +=
                ...
                "</div></div></div>";
            $("#thisIsQuestionDetail").html(html);
...
$("#generateDataTable").click(() => {
	...
	let subject = $("#subselect").val();
    // console.log("hi");
    $.ajax({
        type: "POST",
        url: "/coordinator/getQuestion",
        data: { subject, type, difficulty, createdby:"All", module, mark, topic },
        success: function (response) {
...
function nextPage() {
    ...
    else
    {
        ...
        $.ajax({
            type: "POST",
            url: "/admin/addQuiz/setQuestions",
            data: {selectedQuestions:mySelectedQuestions},
            success: function (response) {
                if(response.next_page)
                {
                    window.open("/admin/addQuiz/setCompulsaryQuestions","_self");
                }
                else {
